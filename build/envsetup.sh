function __print_lmodroid_functions_help() {
cat <<EOF
...
- lmogerrit:       A Git wrapper that fetches/pushes patch from/to LibreMobileOS Gerrit Review.
- lmorebase:       Rebase a Gerrit change and push it again.
- lmoremote:       Add git remote for LibreMobileOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- lmodroidremote:  Add git remote for LMODroid on LibreMobileOS Git.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the LMODroid model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/LMODroid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function lmodroidremote()
{
    ...
    git remote rm lmodroid 2> /dev/null
    ...
    git remote add lmodroid https://git.libremobileos.com/LMODroid/$PROJECT
    echo "Remote 'lmodroid' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.lmodroid.device | grep -q "$LMODROID_BUILD");
    then
...
function lmogerrit() {
    ...
    local user=`git config --get review.gerrit.libremobileos.com.username`
    local review=`git config --get remote.lmodroid.review`
    local project=`git config --get remote.lmodroid.projectname`
...
function lmorebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "LibreMobileOS Gerrit Rebase Usage: "
        echo "      lmorebase <path to project> <patch IDs on Gerrit>"
    ...
    cd $dir
    repo=$(cat .git/config  | grep git://git.libremobileos.com | awk '{ print $NF }' | sed s#git://git.libremobileos.com/##g)
    echo "Starting branch..."
    repo start tmprebase .
    echo "Bringing it up to date..."
    repo sync .
    echo "Fetching change..."
    git fetch "http://gerrit.libremobileos.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lmodroid.device | grep -q "$LMODROID_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
